  ...
  dat <- read.csv("https://raw.githubusercontent.com/bongsongkim/genuine_BLUP/master/raw_data/pheno.csv",header=T)   
...
######
  ...
  kk  <- (ginv(t(z) %*% z) %*% t(z) %*% (y - m.y) %*% t(y - m.y) %*% (z %*% ginv(t(z) %*% z)))/(dim(z)[2] - 1)
